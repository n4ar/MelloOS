...
/// page tables.
#[no_mangle]
pub extern "C" fn ap_entry64(cpu_id: usize) -> ! {
    // Disable interrupts during initialization
    unsafe {
        core::arch::asm!("cli", options(nostack, nomem));
    }
    
    // Get MADT info to find our APIC ID
    let madt_info = get_madt_info().expect("[SMP] AP: Failed to get MADT info");
    
    // Find our APIC ID from the CPU list
    let apic_id = if cpu_id < madt_info.cpus.len() {
        if let Some(cpu_info) = madt_info.cpus[cpu_id] {
            cpu_info.apic_id
        } else {
            panic!("[SMP] AP: CPU ID {} not found in MADT", cpu_id);
        }
    } else {
        panic!("[SMP] AP: Invalid CPU ID: {}", cpu_id);
    };
...
pub fn init_smp(lapic: &mut LocalApic) -> Result<usize, &'static str> {
    ...
    // Create a temporary PMM for page table allocation
    // Note: In a real implementation, we should use the global PMM
    // For now, we'll skip the identity mapping as it may already exist
    // from the bootloader's page tables
    
    // TODO: Properly integrate with global PMM
    // For now, we'll assume the bootloader has already identity-mapped low memory
    serial_println!("[SMP] Skipping identity mapping (assuming bootloader setup)");
    ...
    for cpu_opt in &madt_info.cpus {
        ...
        // Allocate stack for this AP (16KB)
        let stack_top = {
            // Allocate stack memory
            let stack_base = crate::mm::allocator::kmalloc(AP_STACK_SIZE);
            if stack_base.is_null() {
                return Err("Failed to allocate AP stack");
            }
            stack_base as usize + AP_STACK_SIZE
        };
