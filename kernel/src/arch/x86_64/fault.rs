...
pub extern "C" fn page_fault_handler(error_code: u64, fault_addr: u64, rip: u64) {
    ...
    let actual_fault_addr = if fault_addr == 0 { cr2_addr } else { fault_addr };
    
    serial_println!("[FAULT][cpu{}] Page fault at RIP=0x{:x}, fault_addr=0x{:x}, error=0x{:x}", 
                   cpu_id, rip, actual_fault_addr, error_code);
    ...
    serial_println!("[FAULT] Error details: present={}, write={}, user={}, reserved={}, instr_fetch={}", 
                   present, write, user_mode, reserved, instruction_fetch);
...
fn handle_user_page_fault(fault_addr: u64, error_code: u64, rip: u64) {
    ...
    let current_task_info = match sched::get_current_task_info() {
        Some(info) => info,
        None => {
            serial_println!("[FAULT][cpu{}] User page fault but no current task!", cpu_id);
    ...
    serial_println!("[FAULT][cpu{}] User process {} page fault:", cpu_id, current_task_id);
    ...
    if fault_addr >= USER_LIMIT as u64 {
        serial_println!("[FAULT]   Fault address 0x{:x} is outside user space limit 0x{:x}", 
                       fault_addr, USER_LIMIT);
    }
    
    // Get current task to check memory regions
    if let Some(current_task) = sched::get_task_mut(current_task_id) {
        serial_println!("[FAULT]   Task '{}' has {} memory regions:", 
                       current_task.name, current_task.region_count);
        
        // Log all memory regions for debugging
        for i in 0..current_task.region_count {
            if let Some(region) = &current_task.memory_regions[i] {
                serial_println!("[FAULT]     Region {}: 0x{:x}-0x{:x} ({:?})", 
                               i, region.start, region.end, region.region_type);
            }
        }
        
        // Check if fault address is within any valid region
        let in_valid_region = current_task.find_memory_region(fault_addr as usize).is_some();
        
        if !in_valid_region {
            serial_println!("[FAULT]   Fault address 0x{:x} is not in any valid memory region", fault_addr);
        } else {
            serial_println!("[FAULT]   Fault address 0x{:x} is within a valid region (permission violation)", fault_addr);
        }
    }
    
    // Terminate the process
    serial_println!("[FAULT] Terminating process {} due to page fault", current_task_id);
...
fn handle_kernel_page_fault(fault_addr: u64, error_code: u64, rip: u64) -> ! {
    ...
    if let Some((task_id, _)) = sched::get_current_task_info() {
        ...
        if let Some(task) = sched::get_task_mut(task_id) {
            ...
            serial_println!("[FAULT] Task stack: 0x{:x} (size: {})", 
                           task.stack as u64, task.stack_size);
        }
    } else {
        serial_println!("[FAULT] No current task (early boot or idle)");
    }
    
    panic!("[FAULT] Kernel page fault at 0x{:x} from RIP 0x{:x}", fault_addr, rip);
}
