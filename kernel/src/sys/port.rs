...
use super::ipc::{Message, IpcError};
...
impl PortManager {
    ...
    pub fn send_message(&mut self, port_id: usize, data: &[u8]) -> Result<(), IpcError> {
        ...
        if port.is_queue_full() {
            ...
            crate::sys::METRICS.ipc_queue_full.fetch_add(1, Ordering::Relaxed);
        ...
        crate::sys::METRICS.ipc_sends.fetch_add(1, Ordering::Relaxed);
    ...
    pub fn recv_message(
        ...
        if let Some(message) = port.queue.pop_front() {
            ...
            serial_println!("[IPC] Received {} bytes from port {}", bytes_to_copy, port_id);
            ...
            crate::sys::METRICS.ipc_recvs.fetch_add(1, Ordering::Relaxed);
            
            return Ok(bytes_to_copy);
        }
        
        // No message available - block the task
        serial_println!("[IPC] Task {} blocking on port {} (no messages)", task_id, port_id);
