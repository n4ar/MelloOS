...
impl SpaceAllocator {
    ...
    fn split_and_allocate(&mut self, start_lba: u64, blocks: u32) -> Option<FreeExtent> {
        ...
        if extent.length > blocks {
            let remaining =
                FreeExtent::new(extent.start_lba + blocks as u64, extent.length - blocks);
    ...
    pub fn commit_delayed_alloc(&mut self, alloc_id: usize) -> Result<FreeExtent, &'static str> {
        ...
        if let Some(extent) = self.pending[alloc_id].allocated {
            return Ok(extent);
        }

        // Get the number of blocks needed
        let blocks = self.pending[alloc_id].blocks;

        // Allocate physical blocks
        let extent = self.allocate(blocks).ok_or("Failed to allocate blocks")?;

        // Store the allocated extent
        self.pending[alloc_id].allocated = Some(extent);
    ...
    fn coalesce_at(&mut self, start_lba: u64) {
        ...
        if let Some(&next_extent) = self.free_extents.get(&next_lba) {
            ...
            let merged = FreeExtent::new(extent.start_lba, extent.length + next_extent.length);
        ...
        if let Some((&prev_lba, &prev_extent)) = self.free_extents.range(..start_lba).next_back() {
            if prev_extent.end_lba() == start_lba {
                ...
                // Create merged extent
                let merged =
                    FreeExtent::new(prev_extent.start_lba, prev_extent.length + extent.length);
