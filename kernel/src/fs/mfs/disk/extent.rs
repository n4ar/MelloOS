...
impl ExtentManager {
    ...
    pub fn get_all_extents(&self, ino: u64) -> Vec<(u64, ExtentVal)> {
        self.cache
            .get(&ino)
            .map(|extents| {
                extents.iter().map(|(&offset, &val)| (offset, val)).collect()
            })
            .unwrap_or_default()
    }
    ...
    pub fn total_blocks(&self, ino: u64) -> u64 {
        self.cache
            .get(&ino)
            .map(|extents| {
                extents.values().map(|e| e.length as u64).sum()
            })
            .unwrap_or(0)
    }
}
