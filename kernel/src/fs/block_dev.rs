...
use crate::drivers::block::virtio_blk::{block_read, block_write, BlockError as VirtioBlockError};
...
impl From<VirtioBlockError> for BlockError {
    fn from(err: VirtioBlockError) -> Self {
        match err {
            VirtioBlockError::IoError => BlockError::IoError,
            VirtioBlockError::InvalidLba => BlockError::InvalidSector,
            VirtioBlockError::BufferTooSmall => BlockError::BufferTooSmall,
            VirtioBlockError::DeviceNotReady => BlockError::DeviceNotReady,
        }
    }
}
...
impl BlockDevice for VirtioBlockDevice {
    ...
    fn read_sectors(&self, sector: u64, count: u32, buffer: &mut [u8]) -> Result<(), BlockError> {
        ...
        // Read sectors one at a time using virtio-blk driver
        for i in 0..count {
            let sector_offset = (i as usize) * (self.sector_size as usize);
            let sector_buf = &mut buffer[sector_offset..sector_offset + self.sector_size as usize];

            // Call virtio-blk driver's block_read function
            block_read(sector + i as u64, sector_buf).map_err(|e| BlockError::from(e))?;
        }
    ...
    fn write_sectors(&self, sector: u64, count: u32, buffer: &[u8]) -> Result<(), BlockError> {
        ...
        // Write sectors one at a time using virtio-blk driver
        for i in 0..count {
            let sector_offset = (i as usize) * (self.sector_size as usize);
            let sector_buf = &buffer[sector_offset..sector_offset + self.sector_size as usize];

            // Call virtio-blk driver's block_write function
            block_write(sector + i as u64, sector_buf).map_err(|e| BlockError::from(e))?;
        }
    ...
    fn flush(&self) -> Result<(), BlockError> {
        // VirtIO block device doesn't have explicit flush command
        // Writes are synchronous, so this is a no-op
        Ok(())
    }
...
pub fn register_virtio_block_device(device_id: u64, name: &'static str, sector_count: u64) -> u64 {
    ...
    crate::serial_println!(
        "[BLOCK] Registered VirtIO block device '{}' with {} sectors",
        name,
        sector_count
    );
    id
