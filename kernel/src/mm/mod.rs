...
pub fn run_memory_tests(
    pmm: &mut pmm::PhysicalMemoryManager,
    mapper: &mut paging::PageMapper,
) {
    // Print test header
...
pub fn init_memory() {
    ...
    let mut pmm = pmm::PhysicalMemoryManager::init(
        memory_map_response,
        kernel_start,
        kernel_end,
    );
    ...
    mapper.map_kernel_sections(kernel_addr_response, &mut pmm)
    ...
    // Store PMM in a global static for later use
    // This is a simplified approach - in a real kernel you'd want better management
    static PMM: Mutex<Option<pmm::PhysicalMemoryManager>> = Mutex::new(None);
    *PMM.lock() = Some(pmm);
