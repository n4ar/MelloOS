...
pub fn main(argv: &'static [&'static str]) -> Result<i32> {
    ...
    // Print header
    if user_format {
        syscalls::write(1, b"USER       PID  PPID  PGID   SID TTY      STAT   TIME COMMAND\n");
    } else {
...
fn read_proc_stat(pid: usize) -> Result<ProcessInfo> {
    ...
    // Parse stat file
    let stat_str = core::str::from_utf8(&buf[..nread as usize]).map_err(|_| Error::InvalidArgument)?;
    parse_stat(stat_str)
}
...
fn print_process_simple(info: &ProcessInfo) {
    ...
    let padding = if pid_str.len() < 5 { 5 - pid_str.len() } else { 0 };
    for _ in 0..padding {
        buf[pos] = b' ';
        pos += 1;
    }
...
fn print_process_user_format(info: &ProcessInfo) {
    ...
    let padding = if ppid_str.len() < 5 { 5 - ppid_str.len() } else { 0 };
    for _ in 0..padding {
        buf[pos] = b' ';
        pos += 1;
    }
    ...
    let padding = if pgid_str.len() < 5 { 5 - pgid_str.len() } else { 0 };
    for _ in 0..padding {
        buf[pos] = b' ';
        pos += 1;
    }
    ...
    let padding = if sid_str.len() < 5 { 5 - sid_str.len() } else { 0 };
    for _ in 0..padding {
        buf[pos] = b' ';
        pos += 1;
    }
